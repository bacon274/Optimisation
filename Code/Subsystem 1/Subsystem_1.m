...
function run_fmincon(P)
    % This function uses Gradient based optimisation solver to find
    % optimisers. It then passes those optimisers to the Test function
    % which displays and records results 
    P_area = P(1);
...
function run_ga(P)
    % This function uses Genetic Algorithm optimisation solver to find
    % optimisers. It then passes those optimisers to the Test function
    % which displays and records results 
...
function run_swarm(P)
    % This function uses Particle Swarm optimisation solver to find
    % optimisers. It then passes those optimisers to the Test function
    % which displays and records results 
...
function run_multiobjectga(P)
    % This function uses Multi Objective GA optimisation solver to find
    % optimisers. It then passes those optimisers to the Test function
    % which displays and records results 
    P_area = P(1);
    P_power_ub = [P(2);P(2);P(2);P(2);P(2);];
    ...
    %Objective Function
    f = @(x) [Array_cost(x,P_area,P_cost); -Array_energy(x); ]; %objective(x,P_area,P_cost) SO(x,P_to_t_coef,P_area,P_cost)
    %Constraint Fucntion
...
function [c,ceq] = confuneq(x,P_to_t_coef,P_area,P_cost)
    % This function creates the non linear inequality constraints required
    % for optimisation solvers
...
function z = objective(x,P_area,P_cost) % x1-5 = power rating  % need to add in panel information and hence work out how many panels per window
    % This function is the objective function which calculates the number
    % of years until ROI given an array configuration
...
function z = SO(x,P_to_t_coef,P_area,P_cost)
    % this function is an objective function with constreaints added as a
    % penalty value instead of as a seperate function.
...
function e = Array_energy(x)
    % This function calculates the yearly energy output of the array given
    % x values
    global a1 a2 a3 a4 a5 i1 i2 i3 i4 i5
    e = ((x(1)*i1*a1)+(x(2)*i2*a2)+(x(3)*i3*a3)+(x(4)*i4*a4)+(x(5)*i5*a5));
end
function f = FIT_payback(e)
    % This function calculates the yearly payback of the array given
    % x values
    f = e*0.386;
end
function y = Years(c,f)
    % This function calculates the number of years until ROI of the array given
    % x values
    y = c/f;
end
function c = Array_cost(x,P_area,P_cost)
    % This function calculates the cost of the array given
    % x values
...
function hrs = Light(x,P_to_t_coef)
    % This function calculates the hours of quality light allowed by
    % of the array given x values
...
function sensitivity(x,P)
    % This function tests the sensitivity of a given solution with a -10%
    % pertubation
...
function [T] = Test(x,P_to_t_coef,P_area,P_cost,time,solver)
    % This function calculates all the parameters and outputs, plots the
    % solver explored solution space and adds solutions to the solution
    % table
...
% House Keeping
function Clean()
% this function cleans the results so that a new solver can be recorded
